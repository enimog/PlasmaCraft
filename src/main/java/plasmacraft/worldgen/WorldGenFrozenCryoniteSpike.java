...
public class WorldGenFrozenCryoniteSpike extends WorldGenerator
{
    private final Block block;
    
    public WorldGenFrozenCryoniteSpike(Block blockIn)
    {
        block = blockIn;
    }
                ...
                for (int i1 = -l; i1 <= l; ++i1)
                {
                    ...
                    for (int j1 = -l; j1 <= l; ++j1)
                    {
                        ...
                        if ((i1 == 0 && j1 == 0 || f1 * f1 + f2 * f2 <= f * f) && (i1 != -l && i1 != l && j1 != -l && j1 != l || rand.nextFloat() <= 0.75F))
                        {
                            ...
                            {
                                this.setBlockAndNotifyAdequately(worldIn, position.add(i1, k, j1), block.getDefaultState());
                            }
            ...
            for (int l1 = -k1; l1 <= k1; ++l1)
            {
                for (int i2 = -k1; i2 <= k1; ++i2)
                {
                    ...
                    while (blockpos.getY() > 50)
                    {
                        ...
                        if (!iblockstate1.getBlock().isAir(iblockstate1, worldIn, blockpos) && block1 != Blocks.DIRT && block1 != Blocks.SNOW && block1 != Blocks.ICE && block1 != block)
                        {
                            break;
                        }
